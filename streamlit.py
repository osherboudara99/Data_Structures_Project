...
if len(ll_ids) == 0:
    node_value = st.number_input('Enter value to create first node of your first Linked List')
else:
    slider_mark = '' 
    if len(ll_ids) not in [0, 1]:
        slider_mark = f' at/to/from Linked List at ID {slider_response}'

    ops_options = ['Create new Linked List', f'Append{slider_mark}', f'Prepend{slider_mark}', 
                   f'Pop{slider_mark}', f'Pop First{slider_mark}', f'Insert{slider_mark}', 
                   f'Remove{slider_mark}', f'Set{slider_mark}']
    st.selectbox('Select an operation to perform on a Linked List', options = ['Create new Linked List', f'Append{slider_mark}', f'Prepend{slider_mark}', ''])
